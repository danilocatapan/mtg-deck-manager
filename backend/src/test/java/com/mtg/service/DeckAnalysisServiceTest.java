...
class DeckAnalysisServiceTest {
    ...
    void calculatesCmcAverageAndCounts() {
        ...
        when(cardService.searchByName("Sol Ring")).thenReturn(List.of(new CardResponseDTO("Sol Ring","{1}","Artifact","{T}: Add {C}{C}.",1.0)));
        when(cardService.searchByName("Opt")).thenReturn(List.of(new CardResponseDTO("Opt","{U}","Instant","Draw a card.",1.0)));
